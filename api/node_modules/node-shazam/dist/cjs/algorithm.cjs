...
const fft_js_1 = require("./fft.cjs");
const signatures_js_1 = require("./signatures.cjs");
...
class SignatureGenerator {
    initFields() {
        ...
        this.nextSignature = new signatures_js_1.DecodedMessage();
    ...
    doFFT(batchOf128S16leMonoSamples) {
        ...
        const results = (0, fft_js_1.fft)(excerptFromRingBuffer.map((v, i) => (new fft_js_1.ComplexNumber(v * HANNING_MATRIX[i], 0))))
                ...
                if (fftMinus46[binPosition] > maxNeighborInFftMinus49) {
                    ...
                    if (fftMinus46[binPosition] > maxNeighborInOtherAdjacentFFTs) {
                        ...
                        if (frequencyHz < 250) {
                            ...
                            band = signatures_js_1.FrequencyBand._250_520;
                        }
                        else if (frequencyHz <= 1450) {
                            band = signatures_js_1.FrequencyBand._520_1450;
                        }
                        else if (frequencyHz <= 3500) {
                            band = signatures_js_1.FrequencyBand._1450_3500;
                        }
                        else if (frequencyHz <= 5500) {
                            band = signatures_js_1.FrequencyBand._3500_5500;
                        }
                        else
                            continue;
                        if (!Object.keys(this.nextSignature.frequencyBandToSoundPeaks).includes(signatures_js_1.FrequencyBand[band])) {
                            this.nextSignature.frequencyBandToSoundPeaks[signatures_js_1.FrequencyBand[band]] = [];
                        }
                        this.nextSignature.frequencyBandToSoundPeaks[signatures_js_1.FrequencyBand[band]].push(new signatures_js_1.FrequencyPeak(fftNumber, Math.round(peakMagnitude), Math.round(correctedPeakFrequencyBin), 16000));
                    }
                }
            }
        }
